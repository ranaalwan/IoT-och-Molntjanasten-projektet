...
static void sendTelemetry() {
  ...
  // ---- //
  // Send the JSON buffer
  // Added by Johan Holmberg
  //mqtt_client.publish(telemetry_topic, buffer, false);
  mqtt_client.publish("devices/esp82/messages/events/$.ct=application%2Fjson%3Bcharset%3Dutf-8", buffer, false);
  // ---- //
...
void loop() {
  ...
  // Your existing logic for motion detection
  if (PIR == HIGH  &&  MIC > 50) {
    digitalWrite(LED_PIN, HIGH);
    delay(2000);
    digitalWrite(LED_PIN, LOW);
